FROM condaforge/miniforge3:latest
LABEL io.github.snakemake.containerized="true"
LABEL io.github.snakemake.conda_env_hash="6527f29be6f05e32c4044f0c582896d43a2320608f780608f22bbd22780325ec"

# Step 2: Retrieve conda environments

# Conda environment:
#   source: envs/environment.yaml
#   prefix: /conda-envs/7fd07215a201f0b40810f8deac5b018b
#   # SPDX-FileCopyrightText: Contributors to PyPSA-Eur <https://github.com/pypsa/pypsa-eur>
#   #
#   # SPDX-License-Identifier: CC0-1.0
#   
#   name: pypsa-eur
#   channels:
#   - conda-forge
#   - bioconda
#   dependencies:
#   - python>=3.10
#   - pip
#   
#   # Inhouse packages
#   - pypsa>=0.32.1
#   - atlite>=0.3
#   - linopy>=0.4.4
#   - powerplantmatching>=0.5.15
#   
#   # Dependencies of the workflow itself
#   - dask
#   - xlrd
#   - openpyxl
#   - seaborn
#   - snakemake-minimal<8.25 # See https://github.com/snakemake/snakemake/issues/3202
#   - memory_profiler
#   - yaml
#   - pytables
#   - lxml
#   - numpy
#   - pandas>=2.1
#   - geopandas>=1
#   - xarray>=2024.03.0
#   - rioxarray
#   - netcdf4
#   - libgdal-netcdf
#   - networkx
#   - scipy
#   - glpk
#   - shapely>=2.0
#   - matplotlib
#   - proj
#   - fiona
#   - country_converter
#   - geopy
#   - tqdm
#   - pytz
#   - jpype1
#   - pyxlsb
#   - graphviz
#   - geojson
#   
#   # GIS dependencies:
#   - cartopy
#   - descartes
#   - rasterio
#   
#   # Development dependencies
#   - jupyter
#   - ipython
#   - pre-commit
#   - ruff
#   - pylint
#   
#   - pip:
#     - gurobipy
#     - highspy
#     - pyscipopt # See https://github.com/scipopt/PySCIPOpt/issues/944
#     - tsam>=2.3.1
#     - entsoe-py
#     - snakemake-storage-plugin-http
#     - snakemake-executor-plugin-slurm
#     - snakemake-executor-plugin-cluster-generic
RUN mkdir -p /conda-envs/7fd07215a201f0b40810f8deac5b018b
COPY envs/environment.yaml /conda-envs/7fd07215a201f0b40810f8deac5b018b/environment.yaml

# Conda environment:
#   source: rules/MA/envs/pypsa-eur.yaml
#   prefix: /conda-envs/05ed8144f160a2d047529e77a3f204ed
#   # SPDX-FileCopyrightText: Contributors to PyPSA-Eur <https://github.com/pypsa/pypsa-eur>
#   # SPDX-License-Identifier: CC0-1.0
#   
#   name: pypsa-eur
#   channels:
#     - conda-forge
#     - bioconda
#   dependencies:
#     - _libgcc_mutex=0.1
#     - _openmp_mutex=4.5
#     - adwaita-icon-theme=48.0
#     - affine=2.4.0
#     - alsa-lib=1.2.13
#     - amply=0.1.6
#     - anyio=4.8.0
#     - aom=3.9.1
#     - appdirs=1.4.4
#     - argon2-cffi=23.1.0
#     - argon2-cffi-bindings=21.2.0
#     - argparse-dataclass=2.0.0
#     - arrow=1.3.0
#     - astroid=3.3.9
#     - asttokens=3.0.0
#     - async-lru=2.0.4
#     - at-spi2-atk=2.38.0
#     - at-spi2-core=2.40.3
#     - atk-1.0=2.38.0
#     - atlite=0.4.0
#     - attrs=25.3.0
#     - aws-c-auth=0.8.6
#     - aws-c-cal=0.8.7
#     - aws-c-common=0.12.0
#     - aws-c-compression=0.3.1
#     - aws-c-event-stream=0.5.4
#     - aws-c-http=0.9.4
#     - aws-c-io=0.17.0
#     - aws-c-mqtt=0.12.2
#     - aws-c-s3=0.7.13
#     - aws-c-sdkutils=0.2.3
#     - aws-checksums=0.2.3
#     - aws-crt-cpp=0.31.0
#     - aws-sdk-cpp=1.11.510
#     - azure-core-cpp=1.14.0
#     - azure-identity-cpp=1.10.0
#     - azure-storage-blobs-cpp=12.13.0
#     - azure-storage-common-cpp=12.8.0
#     - azure-storage-files-datalake-cpp=12.12.0
#     - babel=2.17.0
#     - beautifulsoup4=4.13.3
#     - bleach=6.2.0
#     - bleach-with-css=6.2.0
#     - blosc=1.21.6
#     - bokeh=3.7.0
#     - bottleneck=1.4.2
#     - branca=0.8.1
#     - brotli=1.1.0
#     - brotli-bin=1.1.0
#     - brotli-python=1.1.0
#     - bzip2=1.0.8
#     - c-ares=1.34.4
#     - c-blosc2=2.17.0
#     - ca-certificates=2025.1.31
#     - cached-property=1.5.2
#     - cached_property=1.5.2
#     - cairo=1.18.4
#     - cartopy=0.24.0
#     - cdsapi=0.7.5
#     - certifi=2025.1.31
#     - cffi=1.17.1
#     - cfgv=3.3.1
#     - cftime=1.6.4
#     - charset-normalizer=3.4.1
#     - click=8.1.8
#     - click-plugins=1.1.1
#     - cligj=0.7.2
#     - cloudpickle=3.1.1
#     - coin-or-cbc=2.10.12
#     - coin-or-cgl=0.60.9
#     - coin-or-clp=1.17.10
#     - coin-or-osi=0.108.11
#     - coin-or-utils=2.11.12
#     - coincbc=2.10.12
#     - colorama=0.4.6
#     - comm=0.2.2
#     - conda-inject=1.3.2
#     - configargparse=1.7
#     - connection_pool=0.0.3
#     - contourpy=1.3.1
#     - country_converter=1.2
#     - cycler=0.12.1
#     - cyrus-sasl=2.1.27
#     - cytoolz=1.0.1
#     - dask=2025.2.0
#     - dask-core=2025.2.0
#     - datapi=0.3.0
#     - datrie=0.8.2
#     - dav1d=1.2.1
#     - dbus=1.13.6
#     - debugpy=1.8.13
#     - decorator=5.2.1
#     - defusedxml=0.7.1
#     - deprecation=2.1.0
#     - descartes=1.1.0
#     - dill=0.3.9
#     - distlib=0.3.9
#     - distributed=2025.2.0
#     - docutils=0.21.2
#     - double-conversion=3.3.1
#     - dpath=2.2.0
#     - entsoe-py=0.6.18
#     - epoxy=1.5.10
#     - et_xmlfile=2.0.0
#     - exceptiongroup=1.2.2
#     - executing=2.1.0
#     - expat=2.6.4
#     - filelock=3.18.0
#     - fiona=1.10.1
#     - folium=0.19.5
#     - font-ttf-dejavu-sans-mono=2.37
#     - font-ttf-inconsolata=3.000
#     - font-ttf-source-code-pro=2.038
#     - font-ttf-ubuntu=0.83
#     - fontconfig=2.15.0
#     - fonts-conda-ecosystem=1
#     - fonts-conda-forge=1
#     - fonttools=4.56.0
#     - fqdn=1.5.1
#     - freetype=2.13.3
#     - freexl=2.0.0
#     - fribidi=1.0.10
#     - fsspec=2025.3.0
#     - gdk-pixbuf=2.42.12
#     - geographiclib=2.0
#     - geojson=3.2.0
#     - geopandas=1.0.1
#     - geopandas-base=1.0.1
#     - geopy=2.4.1
#     - geos=3.13.1
#     - geotiff=1.7.4
#     - gflags=2.2.2
#     - giflib=5.2.2
#     - gitdb=4.0.12
#     - gitpython=3.1.44
#     - glib-tools=2.82.2
#     - glog=0.7.1
#     - glpk=5.0
#     - gmp=6.3.0
#     - graphite2=1.3.13
#     - graphviz=12.2.1
#     - gtk3=3.24.43
#     - gts=0.7.6
#     - h11=0.14.0
#     - h2=4.2.0
#     - harfbuzz=10.4.0
#     - hdf4=4.2.15
#     - hdf5=1.14.3
#     - hicolor-icon-theme=0.17
#     - highspy=1.9.0
#     - hpack=4.1.0
#     - httpcore=1.0.7
#     - httpx=0.28.1
#     - humanfriendly=10.0
#     - hyperframe=6.1.0
#     - icu=75.1
#     - identify=2.6.9
#     - idna=3.10
#     - immutables=0.21
#     - importlib-metadata=8.6.1
#     - importlib_resources=6.5.2
#     - iniconfig=2.0.0
#     - ipykernel=6.29.5
#     - ipython=9.0.2
#     - ipython_pygments_lexers=1.1.1
#     - ipywidgets=8.1.5
#     - isoduration=20.11.0
#     - isort=6.0.1
#     - jedi=0.19.2
#     - jinja2=3.1.6
#     - joblib=1.4.2
#     - jpype1=1.5.2
#     - json-c=0.18
#     - json5=0.10.0
#     - jsonpointer=3.0.0
#     - jsonschema=4.23.0
#     - jsonschema-specifications=2024.10.1
#     - jsonschema-with-format-nongpl=4.23.0
#     - jupyter=1.1.1
#     - jupyter-lsp=2.2.5
#     - jupyter_client=8.6.3
#     - jupyter_console=6.6.3
#     - jupyter_core=5.7.2
#     - jupyter_events=0.12.0
#     - jupyter_server=2.15.0
#     - jupyter_server_terminals=0.5.3
#     - jupyterlab=4.3.6
#     - jupyterlab_pygments=0.3.0
#     - jupyterlab_server=2.27.3
#     - jupyterlab_widgets=3.0.13
#     - keyutils=1.6.1
#     - kiwisolver=1.4.8
#     - krb5=1.21.3
#     - lcms2=2.17
#     - ld_impl_linux-64=2.43
#     - lerc=4.0.0
#     - libabseil=20250127.0
#     - libaec=1.1.3
#     - libarchive=3.7.7
#     - libarrow=19.0.1
#     - libarrow-acero=19.0.1
#     - libarrow-dataset=19.0.1
#     - libarrow-substrait=19.0.1
#     - libavif16=1.2.0
#     - libblas=3.9.0
#     - libbrotlicommon=1.1.0
#     - libbrotlidec=1.1.0
#     - libbrotlienc=1.1.0
#     - libcblas=3.9.0
#     - libclang-cpp19.1=19.1.7
#     - libclang13=19.1.7
#     - libcrc32c=1.1.2
#     - libcups=2.3.3
#     - libcurl=8.12.1
#     - libde265=1.0.15
#     - libdeflate=1.23
#     - libdrm=2.4.124
#     - libedit=3.1.20250104
#     - libegl=1.7.0
#     - libev=4.33
#     - libevent=2.1.12
#     - libexpat=2.6.4
#     - libffi=3.4.6
#     - libgcc=14.2.0
#     - libgcc-ng=14.2.0
#     - libgd=2.3.3
#     - libgdal-core=3.10.2
#     - libgdal-hdf4=3.10.2
#     - libgdal-hdf5=3.10.2
#     - libgdal-netcdf=3.10.2
#     - libgfortran=14.2.0
#     - libgfortran5=14.2.0
#     - libgl=1.7.0
#     - libglib=2.82.2
#     - libglvnd=1.7.0
#     - libglx=1.7.0
#     - libgomp=14.2.0
#     - libgoogle-cloud=2.36.0
#     - libgoogle-cloud-storage=2.36.0
#     - libgrpc=1.71.0
#     - libheif=1.19.7
#     - libiconv=1.18
#     - libjpeg-turbo=3.0.0
#     - libkml=1.3.0
#     - liblapack=3.9.0
#     - liblapacke=3.9.0
#     - libllvm19=19.1.7
#     - liblzma=5.6.4
#     - libnetcdf=4.9.2
#     - libnghttp2=1.64.0
#     - libnsl=2.0.1
#     - libntlm=1.8
#     - libopenblas=0.3.29
#     - libopengl=1.7.0
#     - libopentelemetry-cpp=1.18.0
#     - libopentelemetry-cpp-headers=1.18.0
#     - libparquet=19.0.1
#     - libpciaccess=0.18
#     - libpng=1.6.47
#     - libpq=17.4
#     - libprotobuf=5.29.3
#     - libre2-11=2024.07.02
#     - librsvg=2.58.4
#     - librttopo=1.1.0
#     - libsodium=1.0.20
#     - libspatialite=5.1.0
#     - libsqlite=3.49.1
#     - libssh2=1.11.1
#     - libstdcxx=14.2.0
#     - libstdcxx-ng=14.2.0
#     - libthrift=0.21.0
#     - libtiff=4.7.0
#     - libutf8proc=2.10.0
#     - libuuid=2.38.1
#     - libwebp-base=1.5.0
#     - libxcb=1.17.0
#     - libxcrypt=4.4.36
#     - libxkbcommon=1.8.1
#     - libxml2=2.13.6
#     - libxslt=1.1.39
#     - libzip=1.11.2
#     - libzlib=1.3.1
#     - linopy=0.5.0
#     - locket=1.0.0
#     - lxml=5.3.1
#     - lz4=4.3.3
#     - lz4-c=1.10.0
#     - lzo=2.10
#     - mapclassify=2.8.1
#     - markupsafe=3.0.2
#     - matplotlib=3.10.1
#     - matplotlib-base=3.10.1
#     - matplotlib-inline=0.1.7
#     - mccabe=0.7.0
#     - memory_profiler=0.61.0
#     - minizip=4.0.7
#     - mistune=3.1.2
#     - msgpack-python=1.1.0
#     - multiurl=0.3.3
#     - munkres=1.1.4
#     - mysql-common=9.0.1
#     - mysql-libs=9.0.1
#     - narwhals=1.30.0
#     - nbclient=0.10.2
#     - nbconvert-core=7.16.6
#     - nbformat=5.10.4
#     - ncurses=6.5
#     - nest-asyncio=1.6.0
#     - netcdf4=1.7.2
#     - networkx=3.4.2
#     - nlohmann_json=3.11.3
#     - nodeenv=1.9.1
#     - nomkl=1.0
#     - notebook=7.3.3
#     - notebook-shim=0.2.4
#     - numexpr=2.10.2
#     - numpy=1.26.4
#     - openjpeg=2.5.3
#     - openldap=2.6.9
#     - openpyxl=3.1.5
#     - openssl=3.4.1
#     - orc=2.1.1
#     - overrides=7.7.0
#     - packaging=24.2
#     - pandas=2.2.3
#     - pandocfilters=1.5.0
#     - pango=1.56.2
#     - parso=0.8.4
#     - partd=1.4.2
#     - patsy=1.0.1
#     - pcre2=10.44
#     - pexpect=4.9.0
#     - pickleshare=0.7.5
#     - pillow=11.1.0
#     - pip=25.0.1
#     - pixman=0.44.2
#     - pkgutil-resolve-name=1.3.10
#     - plac=1.4.3
#     - platformdirs=4.3.6
#     - pluggy=1.5.0
#     - polars=1.24.0
#     - powerplantmatching=0.7.1
#     - pre-commit=4.1.0
#     - progressbar2=4.5.0
#     - proj=9.5.1
#     - prometheus-cpp=1.3.0
#     - prometheus_client=0.21.1
#     - prompt-toolkit=3.0.50
#     - prompt_toolkit=3.0.50
#     - psutil=7.0.0
#     - pthread-stubs=0.4
#     - ptyprocess=0.7.0
#     - pulp=2.8.0
#     - pure_eval=0.2.3
#     - py-cpuinfo=9.0.0
#     - pyarrow=19.0.1
#     - pyarrow-core=19.0.1
#     - pycountry=24.6.1
#     - pycparser=2.22
#     - pygments=2.19.1
#     - pylint=3.3.5
#     - pyogrio=0.10.0
#     - pyparsing=3.2.1
#     - pyproj=3.7.1
#     - pypsa=0.33.2
#     - pyshp=2.3.1
#     - pyside6=6.8.2
#     - pysocks=1.7.1
#     - pytables=3.10.2
#     - pytest=8.3.5
#     - python=3.12.9
#     - python-dateutil=2.9.0.post0
#     - python-fastjsonschema=2.21.1
#     - python-json-logger=2.0.7
#     - python-tzdata=2025.1
#     - python-utils=3.9.1
#     - python_abi=3.12
#     - pytz=2024.1
#     - pyxlsb=1.0.10
#     - pyyaml=6.0.2
#     - pyzmq=26.3.0
#     - qhull=2020.2
#     - qt6-main=6.8.2
#     - rasterio=1.4.3
#     - rav1e=0.6.6
#     - re2=2024.07.02
#     - readline=8.2
#     - referencing=0.36.2
#     - requests=2.32.3
#     - reretry=0.11.8
#     - rfc3339-validator=0.1.4
#     - rfc3986-validator=0.1.1
#     - rioxarray=0.18.2
#     - rpds-py=0.23.1
#     - ruff=0.11.0
#     - s2n=1.5.14
#     - scikit-learn=1.6.1
#     - scipy=1.15.2
#     - seaborn=0.13.2
#     - seaborn-base=0.13.2
#     - send2trash=1.8.3
#     - setuptools=75.8.2
#     - shapely=2.0.7
#     - six=1.17.0
#     - smart_open=7.1.0
#     - smmap=5.0.2
#     - snakemake-interface-common=1.17.4
#     - snakemake-interface-executor-plugins=9.3.3
#     - snakemake-interface-report-plugins=1.1.0
#     - snakemake-interface-storage-plugins=3.4.0
#     - snakemake-minimal=8.24.1
#     - snappy=1.2.1
#     - sniffio=1.3.1
#     - snuggs=1.4.7
#     - sortedcontainers=2.4.0
#     - soupsieve=2.5
#     - sqlite=3.49.1
#     - stack_data=0.6.3
#     - statsmodels=0.14.4
#     - svt-av1=3.0.1
#     - tabulate=0.9.0
#     - tblib=3.0.0
#     - terminado=0.18.1
#     - threadpoolctl=3.6.0
#     - throttler=1.2.2
#     - tinycss2=1.4.0
#     - tk=8.6.13
#     - tomli=2.2.1
#     - tomlkit=0.13.2
#     - toolz=1.0.0
#     - tornado=6.4.2
#     - tqdm=4.67.1
#     - traitlets=5.14.3
#     - types-python-dateutil=2.9.0.20241206
#     - typing-extensions=4.12.2
#     - typing_extensions=4.12.2
#     - typing_utils=0.1.0
#     - tzdata=2025a
#     - ukkonen=1.0.1
#     - unicodedata2=16.0.0
#     - unidecode=1.3.8
#     - uri-template=1.3.0
#     - uriparser=0.9.8
#     - urllib3=2.3.0
#     - validators=0.34.0
#     - virtualenv=20.29.3
#     - wayland=1.23.1
#     - wcwidth=0.2.13
#     - webcolors=24.11.1
#     - webencodings=0.5.1
#     - websocket-client=1.8.0
#     - wheel=0.45.1
#     - widgetsnbextension=4.0.13
#     - wrapt=1.17.2
#     - x265=3.5
#     - xarray=2025.1.2
#     - xcb-util=0.4.1
#     - xcb-util-cursor=0.1.5
#     - xcb-util-image=0.4.0
#     - xcb-util-keysyms=0.4.1
#     - xcb-util-renderutil=0.3.10
#     - xcb-util-wm=0.4.2
#     - xerces-c=3.2.5
#     - xkeyboard-config=2.43
#     - xlrd=2.0.1
#     - xorg-libice=1.1.2
#     - xorg-libsm=1.2.6
#     - xorg-libx11=1.8.12
#     - xorg-libxau=1.0.12
#     - xorg-libxcomposite=0.4.6
#     - xorg-libxcursor=1.2.3
#     - xorg-libxdamage=1.1.6
#     - xorg-libxdmcp=1.1.5
#     - xorg-libxext=1.3.6
#     - xorg-libxfixes=6.0.1
#     - xorg-libxi=1.8.2
#     - xorg-libxinerama=1.1.5
#     - xorg-libxrandr=1.5.4
#     - xorg-libxrender=0.9.12
#     - xorg-libxtst=1.2.5
#     - xorg-libxxf86vm=1.1.6
#     - xyzservices=2025.1.0
#     - yaml=0.2.5
#     - yte=1.7.0
#     - zeromq=4.3.5
#     - zict=3.0.0
#     - zipp=3.21.0
#     - zlib=1.3.1
#     - zlib-ng=2.2.4
#     - zstandard=0.23.0
#     - zstd=1.5.7
#     - pip:
#         - gurobipy==12.0.1
#         - oauthlib==3.2.2
#         - ply==3.11
#         - pyomo==6.9.1
#         - pyscipopt==5.4.1
#         - requests-oauthlib==1.3.1
#         - snakemake-executor-plugin-cluster-generic==1.0.9
#         - snakemake-executor-plugin-slurm==1.1.0
#         - snakemake-executor-plugin-slurm-jobstep==0.3.0
#         - snakemake-storage-plugin-http==0.2.3
#         - tsam==2.3.6
#     - pypsa
#     - geopandas
#     - shapely
#     - pandas
#     - sqlalchemy
#     - psycopg2
#   prefix: /home/runner/miniconda3/envs/pypsa-eur
RUN mkdir -p /conda-envs/05ed8144f160a2d047529e77a3f204ed
COPY rules/MA/envs/pypsa-eur.yaml /conda-envs/05ed8144f160a2d047529e77a3f204ed/environment.yaml

# Step 3: Generate conda environments

RUN conda env create --prefix /conda-envs/7fd07215a201f0b40810f8deac5b018b --file /conda-envs/7fd07215a201f0b40810f8deac5b018b/environment.yaml && \
    conda env create --prefix /conda-envs/05ed8144f160a2d047529e77a3f204ed --file /conda-envs/05ed8144f160a2d047529e77a3f204ed/environment.yaml && \
    conda clean --all -y
